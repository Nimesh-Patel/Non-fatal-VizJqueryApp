...
const getTooltipData = function (metadata, groupData, colors, cn) {
    ...
    var groupStr = "<span style='color:" + color + "'><strong>" +
...
const assignTrendsLegend = function (data, metadata, getLegendColor) {
    var palettes = app.globals.palettes.gender;
    var names = [];
    if (metadata.id == "#genderTrend") {
        //app.globals.filters.dataReturned.push('genderTrendChart');
    ...
    buildTrendsLegends(metadata.id);
}

const buildTrendsLegends = () => {
    $('#genderTrendsLegend').html('');
    $('#genderTrendsLegend').html(genderLegendTemplate({ legends: app.globals.anc_legends }));
}

const buildGenderTrends = (data, metadata) => {
    ...
    data.forEach(function (d) {
        d.compare = d[metadata.compare],
            ...
            d.date = formats.parseYear(d.year.toString()),
    ...
    // Y AXIS Title Labels
    if (metadata.id == "#genderTrend") {
        if (d3.select("#genderDropdown").property("value") == "injuries") {
    ...
    svg.append("text")
        ...
        .attr("transform", "translate(" + (0 - 62) + "," + (height / 2) + ")rotate(-90)")  // text is drawn off the screen top left, move down and out and rotate
        .text(Ytitle);
    ...
    dataNested.forEach((groupObj) => {
        ...
        sets.append("path")
            ...
            .style("stroke", function (d) {
                if (groupObj.key == "Males") {
                    return app.globals.palettes.gender[1];
                } else if (groupObj.key == "Females") {
        ...
        groupObj.values.forEach((compareObj) => {
            ...
            svg.append("g")
                ...
                .attr("aria-label", function () {
                    if ($('#genderDropdown').val() == 'injuries') {
                        return compareObj.year + ' ' + compareObj.group + ' ' + $('#genderDropdown').val() + ' ' + $.number(compareObj.count, 0);
                    } else {
                        return compareObj.year + ' ' + compareObj.group + ' ' + $('#genderDropdown').val() + ' ' + $.number(compareObj.count, 1);
                    }
                })
                .attr("r", function (d) {
                    return 5;
                ...
                .style("fill", function (d) {
                    ...
                    currentValue = currentValue[0];
                    if (compareObj.group == "Males") {
                        return app.globals.palettes.gender[1];
                    } else if (compareObj.group == "Females") {
                        return app.globals.palettes.gender[0];
                    } else {
                        return app.globals.palettes.gender[2];
                    }
