...
const getTooltipData = function (metadata, groupData, colors, cn) {
    ...
    var countFormat;
    // countFormat = formats.formatCount(groupData.count, metadata.group);
...
const assignTrendsLegend = function (data, metadata, getLegendColor) {
    var palettes = app.globals.palettes.trends;
    var names = [];
    if (metadata.id == "#ageTrend") {
        //app.globals.filters.dataReturned.push('ageTrendChart');
...
const buildTrendsLegends = () => {
    $('#ageGroupTrendsLegend').html('');
    $('#ageGroupTrendsLegend').html(comparisonLegendTemplate({ legends: app.globals.anc_legends }));
}

const buildAgeTrends = (data, metadata) => {
            ...
            svg.append("g")
                ...
                .attr("aria-label", function () {
                    if ($('#ageDropdown').val() == 'injuries') {
                        return compareObj.year + ' ' + compareObj.group + ' ' + $('#ageDropdown').val() + ' ' + $.number(compareObj.count, 0);
                    } else {
                        return compareObj.year + ' ' + compareObj.group + ' ' + $('#ageDropdown').val() + ' ' + $.number(compareObj.count, 1);
                    }
                })
                .attr("r", function (d) {
                    return 5;
